...
When defining a variable, it must be checked, whether \textbf{redefintion} of already
existing variable within the same nested blocks occurs. This is perfomed by searching current stack using
\verb|findInStack()|. When \verb|NULL| is returned, no redefiniton occurs. Similarly,
\verb|funSymtable| is also searched for an entry with matching ID.
\par Assignment cannot be performed on \textbf{undefined variables}. Verification is similar to redefinition check,
however non-\verb|NULL| value is expected.
\par \textbf{Matching data types} when assigning and defining of the variable and the assigned expression must be confirmed.
...
If the data type can be deduced, it is set to symbol table of a variable along with nullability of the variable.
...
With separate symbol table for built-in function, matching IDs are allowed (e.g. \verb|ifj.write()| and user-defined \verb|write()|).
Parameters of functions are also analysed in the same manner.
\par
...
by examining \verb|used| flag in \verb|funSymtable| nodes.
\par
...
At the same time, \textbf{parameters and return type of main} are checked, whether they match with expected ones.
